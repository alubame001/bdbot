...
//   JDBOT_USERNAME=Gggggg JDBOT_PASSWORD=gggggg node bdbot.js
//   node bdbot.js JDBOT_USERNAME=Gggggg JDBOT_PASSWORD=gggggg
// 
// or like this:
//
...
var url = "http://ltc.btcdice.com/welcome/login";
...
login_then_run_bot({hash:     process.env.JDBOT_HASH,
                    login: process.env.JDBOT_USERNAME,
...
function login_then_run_bot(credentials) {
    //console.log("login_then_run_bot:",credentials)
...
function login(credentials, cb) {
    ...
    if (credentials.login) req.form.login = credentials.login;
    ...
    request.post(req, function(err, res, body) {
        ...
        //var cookie = jar.getCookieString("http://ltc.btcdice.com");
        console.log(body)
    ...
    request('http://ltc.btcdice.com', function (error, response, body) {
...
function run_bot(cookie) {
    ...
    socket = require("socket.io-client")("http://ltc.btcdice.com", {transports: [transport],
                                               extraHeaders: {
                                                   origin: "http://ltc.btcdice.com",
                                                   cookie: cookie
                                               }});


  //  socket = require("socket.io-client")("http://ltc.btcdice.com");
